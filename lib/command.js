...
const fs = require('fs');
const PomeloManagerAgent = require('./pomeloManagerAgent');
...
class CommandManager {
  ...
  /**
   *
   * @param {PomeloManagerAgent} agent pomeloManagerAgent
   */
  constructor(agent) {
    this.agent = agent;
  ...
  handle(argv, msg, client, ctx) {
    ...
    if (m) {
      const _command = m(self);
      _command.handleFilter(ctx, comd1, argv, msg);
    } else {
      util.errorHandle(argv, client, ctx);
    }
  }
}
